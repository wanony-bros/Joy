...
private val TWITTER_EXPANSIONS = setOf<String>() // Don't need yet
private val TWITTER_TWEET_FIELDS = setOf("author_id", "id", "created_at")
...
class TwitterCommand : JoyCommand {
    ...
    override val commandData: CommandData = Commands.slash(commandName, "Follow or unfollow a twitter user")
        .addSubcommands(
            SubcommandData(TWITTER_FOLLOW_OPERATION_NAME, "Follow a twitter user")
                .addOption(OptionType.STRING, "name", "The twitter user to follow", true),
            SubcommandData(TWITTER_UNFOLLOW_OPERATION_NAME, "Unfollow a twitter user")
                .addOption(OptionType.STRING, "name", "The twitter user to unfollow", true)
        )
    ...
    private suspend fun followTwitterUser(event: SlashCommandInteractionEvent) {
        ...
        //    launch {
                try {
                    val aOrDRequest = AddRulesRequest().addAddItem(RuleNoId().value("from:irishsanjoy"))
                    val aOrDResponse = twitter.tweets().addOrDeleteRules(AddOrDeleteRulesRequest(aOrDRequest), true)
                    println(aOrDRequest)
                    println(aOrDResponse)
                    val stream = twitter.tweets().searchStream(null, TWITTER_TWEET_FIELDS, null, null, null, null, 5)
                    try {
