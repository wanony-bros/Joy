...
class ClaimPremiumCommand : JoyCommand {
    ...
    private fun claimPremium(event: SlashCommandInteractionEvent) = DB.transaction {
        ...
        if (event.guild!!.idLong != joyServerId) {
            event.replyEmbeds(
                Theme.errorEmbed("This command needs to be used in the Joy support server!")
        ...
        if (event.member!!.roles.any { it.idLong == premiumRoleId || it.idLong == premiumTestRoleId}) {
            ...
            if (!madePremium) {
                event.replyEmbeds(
                    Theme.errorEmbed("Failed to add premium commands! Please contact support in Joy's support server")
                    .build()).setEphemeral(true).queue()
                return@transaction
            }
            ...
            event.replyEmbeds(
                Theme.errorEmbed("No $PREMIUM_SUBSCRIPTION_NAME role found! Have you linked Patreon to your discord account?")
                .build()).setEphemeral(true).queue()
            return@transaction
        }
